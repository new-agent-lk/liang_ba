...
class CompanyRecruitsPage(Page):
    ...
    def get_context(self, request):
        ...
        campus_recruitments = CompanyContentPage.objects.filter(category='Recruits', page_type='校园招聘').live().order_by('first_published_at')
        Social_recruitments = CompanyContentPage.objects.filter(category='Recruits', page_type='社会招聘').live().order_by('first_published_at')
