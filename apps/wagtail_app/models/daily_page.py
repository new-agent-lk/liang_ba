...
from wagtail_app.models.content_page import CompanyDailyContentPage

from website_info.models import CompanyInfo, FriendlyLinks


class CompanyDailyPage(Page):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        context['website_info'] = CompanyInfo.objects.all().order_by('-id')[0]  # 获取最新的一个公司信息对象
        context['friendly_links'] = FriendlyLinks.objects.all()  # 获取所有友情链接对象

        query_set = CompanyDailyContentPage.objects.descendant_of(self).live().order_by("title")
        context['content_page_count'] = len(query_set)
        context['content_pages'] = query_set
        return context


class CompanyDailyPageBannerImage(Orderable):
    page = ParentalKey(CompanyDailyPage, on_delete=models.CASCADE, related_name='banner_images')
