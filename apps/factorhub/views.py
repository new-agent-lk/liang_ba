...
class StockListView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"count": len(stocks), "results": stocks.to_dict("records")})
...
class StockPoolView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"pool": pool_code, "count": len(symbols), "symbols": symbols})
            ...
            while current <= end_date:
                for symbol in symbols[:5]:  # 限制5只股票
                    ...
                    open_price = close_price * (
                        1 + (hash(f"{symbol}{current.date()}") % 200 - 100) / 10000
                    )
...
class FactorListView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"factors": factors, "categories": categories})
