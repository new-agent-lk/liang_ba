...
class ReportListView(ListView):
    ...
    def get_queryset(self):
        # 只返回已发布且公开的报告
        queryset = (
            ResearchReport.objects.filter(is_public=True, status="published")
            .select_related("author")
            .order_by("-is_top", "-published_at")
        )
        ...
        if year:
            months = (
                ResearchReport.objects.filter(
                    is_public=True, status="published", published_at__year=int(year)
                )
                .annotate(month=ExtractMonth("published_at"))
...
class ReportDetailView(DetailView):
    ...
    def get_queryset(self):
        # 只返回已发布且公开的报告
        return ResearchReport.objects.filter(is_public=True, status="published").select_related(
            "author"
        )
