...
class ResearchReport(models.Model):
    ...
    annual_return = models.DecimalField(
        "年化收益率", max_digits=10, decimal_places=2, null=True, blank=True
    )
    max_drawdown = models.DecimalField(
        "最大回撤", max_digits=10, decimal_places=2, null=True, blank=True
    )
    sharpe_ratio = models.DecimalField(
        "夏普比率", max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    equity_curve_image = models.ImageField(
        "资金曲线图", upload_to="reports/equity/%Y/%m/", null=True, blank=True
    )
    drawdown_image = models.ImageField(
        "回撤图", upload_to="reports/drawdown/%Y/%m/", null=True, blank=True
    )
    monthly_returns_image = models.ImageField(
        "月度收益图", upload_to="reports/monthly/%Y/%m/", null=True, blank=True
    )
    ...
    author = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="research_reports", verbose_name="作者"
    )
