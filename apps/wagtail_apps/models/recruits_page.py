...
class CompanyRecruitsPage(Page):
    ...
    campus_intro = models.CharField(
        verbose_name="校园招聘介绍", max_length=255, null=True, blank=True
    )
    social_intro = models.CharField(
        verbose_name="社会招聘介绍", max_length=255, null=True, blank=True
    )
    ...
    def get_context(self, request):
        ...
        campus_recruitments = (
            CompanyContentPage.objects.filter(category="Recruits", page_type="校园招聘")
            .live()
            .order_by("first_published_at")
        )
        Social_recruitments = (
            CompanyContentPage.objects.filter(category="Recruits", page_type="社会招聘")
            .live()
            .order_by("first_published_at")
        )
