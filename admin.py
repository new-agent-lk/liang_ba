...
class SystemLogIndexAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Log Information", {"fields": ("timestamp", "level", "log_type")}),
        ("Content", {"fields": ("logger", "message")}),
        ("Context", {"fields": ("trace_id", "request_id", "user_id", "client_ip")}),
        (
            "Metadata",
            {
...
class LogViewerAccessLogAdmin(admin.ModelAdmin):
    ...
    list_display = ["create_time", "username", "action_colored", "log_type", "ip_address"]
...
class LogAdminSite(admin.AdminSite):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path(
                "logs/file-viewer/<str:log_type>/",
                self.admin_view(self.file_viewer),
                name="log-file-viewer-type",
            ),
            path(
                "logs/rotation-dashboard/",
                self.admin_view(self.rotation_dashboard),
                name="log-rotation-dashboard",
            ),
            path(
                "api/logs/rotate/<str:log_type>/",
                self.admin_view(self.api_rotate_log),
                name="log-api-rotate",
            ),
        ]
