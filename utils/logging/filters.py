...
class SensitiveDataFilter(logging.Filter):
    ...
    FIELD_PATTERN = re.compile(
        rf'({"|".join(SENSITIVE_FIELDS)})["\']?\s*[:=]\s*["\']?([^"\'\s,}}]+)', re.IGNORECASE
    )
    ...
    def _redact_sensitive_data(self, text: str) -> str:
        """脱敏文本中的敏感信息"""
        # 脱敏字段值
        text = self.FIELD_PATTERN.sub(lambda m: f"{m.group(1)}={self.MASK}", text)
...
class RateLimitFilter(logging.Filter):
    ...
    def __init__(self, rate: int = 100, per: int = 60, cache_size: int = 1000, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _cleanup(self, now: float) -> None:
        """清理过期的缓存条目"""
        expired = [
            key for key, (_, first_time) in self._cache.items() if now - first_time > self.per
        ]
        for key in expired:
