...
// 部门选项列表
const DEPARTMENT_ITEMS = Object.entries(DEPARTMENT_CONFIG).map(
  ([key, cfg]) => ({
    key,
    label: (
      <div style={getDeptTagStyle(cfg)}>
        <span>{cfg.label}</span>
      </div>
    ),
  }),
  ...
  const columns = [
    ...
    { title: "职位名称", dataIndex: "title", key: "title", width: 150 },
    {
      ...
      render: (v: string, record: JobPosition) => {
        const config = DEPARTMENT_CONFIG[v] || {
          label: v || "-",
          color: "#666",
          bgColor: "#f5f5f5",
          borderColor: "#d9d9d9",
        ...
        return (
          <Dropdown
            menu={{
              ...
              onClick: ({ key }) => handleEditDepartment(record.id, key),
            }}
            trigger={["click"]}
            placement="bottomLeft"
          >
            <Button size="large" type="text" style={getDeptTagStyle(config)}>
              {config.label}
            </Button>
          </Dropdown>
        );
    ...
    },
    {
      ...
      width: 120,
      render: (v: string, record: JobPosition) => {
        const categoryItem = JOB_CATEGORY_CHOICES.find(
        ...
        return (
          <Dropdown
            menu={{
              ...
              onClick: ({ key }) => handleEditCategory(record.id, key),
            }}
            trigger={["click"]}
            placement="bottomLeft"
          >
    ...
    },
    {
      ...
      width: 120,
      render: (v: string, record: JobPosition) => {
        const typeItem = RECRUITMENT_TYPE_CHOICES.find(
          (item) => item.value === record.recruitment_type,
        );
        const label = typeItem?.label || v || "-";
        return (
          <Dropdown
            menu={{
              items: RECRUITMENT_TYPE_ITEMS,
              onClick: ({ key }) => handleEditRecruitmentType(record.id, key),
            }}
            trigger={["click"]}
            placement="bottomLeft"
          >
    ...
    { title: "薪资范围", dataIndex: "salary_range", key: "salary", width: 100 },
    {
      ...
      render: (v: string, record: JobPosition) => {
        ...
        const menuItems = Object.entries(JOB_STATUS_CONFIG).map(
          ([key, cfg]) => ({
            key,
            label: (
