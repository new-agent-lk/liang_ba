...
import { Card, Row, Col, Form, Select, DatePicker, Button, Table, Space, Statistic, message, Empty, Tag } from 'antd';
import dayjs from 'dayjs';
import { DatabaseOutlined, DownloadOutlined, DeleteOutlined, ReloadOutlined, SyncOutlined } from '@ant-design/icons';
...
const FactorData: React.FC = () => {
  ...
  const handleGetCacheInfo = async () => {
    try {
      ...
      setCacheInfo(response.data || { total_files: 0, total_size_mb: 0 });
    } catch (error) {
    ...
    <div style={{ padding: 24 }}>
      ...
      >
        {data.length > 0 ? (
          ...
          >
            <Button type="primary" icon={<DownloadOutlined />} onClick={handleFetchData} loading={loading}>
              获取数据
            </Button>
          </Empty>
        )}
