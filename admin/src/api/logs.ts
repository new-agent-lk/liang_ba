...
export const updateRotationConfig = (
  data: Partial<LogRotationConfig>,
...
export const getRotationStatus = (
  logType: string,
): Promise<LogRotationStatus> => {
  return request.get("/api/admin/logs/rotation/action/", {
    params: { log_type: logType },
  });
};

// Perform rotation action (rotate, pause, resume)
export const performRotationAction = (
  data: LogRotationAction,
...
export const getArchivedFiles = (
  logType: string,
): Promise<{ archived_files: unknown[]; total_count: number }> => {
  return request.get("/api/admin/logs/rotation/files/", {
    params: { log_type: logType },
  });
};

// Delete archived file
export const deleteArchivedFile = (
  logType: string,
  filename: string,
): Promise<{ status: string }> => {
  return request.delete("/api/admin/logs/rotation/files/", {
    params: { log_type: logType, filename },
  });
};
